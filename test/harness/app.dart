...
export 'package:aqueduct_forum_backend/aqueduct_forum_backend.dart';
export 'package:test/test.dart';

/// A testing harness for aqueduct_forum_backend.
///
/// Use instances of this class to start/stop the test aqueduct_forum_backend server. Use [client] to execute
...
class TestApplication {
  ...
  TestApplication() {
    configuration = new AqueductForumBackendConfiguration("config.yaml.src");
    configuration.database.isTemporary = true;
  }

  Application<AqueductForumBackendSink> application;
  AqueductForumBackendSink get sink => application.mainIsolateSink;
  LoggingServer logger = new LoggingServer([]);
  TestClient client;
  AqueductForumBackendConfiguration configuration;

  /// Starts running this test harness.
  ///
  /// This method will start a [LoggingServer] and an [Application] with [AqueductForumBackendSink].
  ...
  /// used to generate a temporary database schema. The [AqueductForumBackendSink] instance will use
  ...
  Future start() async {
    ...
    application = new Application<AqueductForumBackendSink>();
    application.configuration.configurationOptions = {
      AqueductForumBackendSink.ConfigurationKey: configuration,
